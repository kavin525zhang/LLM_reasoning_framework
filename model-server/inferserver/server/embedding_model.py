...
class FlagEmbeddingModelService:
    ...
    def __init__(self,
        ...
        log.info(f"Start init embedding model {embedding_model_path}")
        self.lock = threading.Lock()
        self.model = BGEM3FlagModel(embedding_model_path, use_fp16=False, devices=device_id)
        self.batch_size = batch_size
        log.info(f"Init embedding model {embedding_model_path} done.")
    ...
    def encode(self, 
        ...
        with self.lock:
            data_embeddings = self.model.encode(query,
                                                ...
                                                return_colbert_vecs=return_colbert_vecs)["dense_vecs"].tolist()
        assert len(query) == len(data_embeddings), \
            f"文本和向量数据不对应, 文本{len(query)}, 向量{len(data_embeddings)}"
    ...
    def compute_score(self,
        ...
        scores = self.model.compute_score_single_device(pairs, 
                                          max_passage_length=max_passage_length, # a smaller max length leads to a lower latency
