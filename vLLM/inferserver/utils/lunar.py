# https://gist.github.com/taojy123/2e27a8912120d975940b1667f4f0105f
import datetime
...
class Lunar(object):
    ...
    def __init__(self, dt=None):
        ...
        self.localtime = dt if dt else datetime.datetime.now(pytz.timezone('Asia/Shanghai'))
    ...
    def _date_diff(self):
        """返回基于1901/01/01日差数"""
        return (self.localtime - datetime.datetime(1901, 1, 1,
                                                   tzinfo=pytz.timezone('Asia/Shanghai'))).days
    ...
    def solar_weekday(self):
        weekday_map = ["一", "二", "三", "四", "五", "六", "日"]
        return "星期" + weekday_map[self.localtime.weekday()]

    def __str__(self):
        self.localtime.weekday()
        ret = f"公历 {self.localtime.date()} 北京时间 {self.localtime.strftime('%H:%M')} {self.solar_weekday()} "
        ret += f"{self.ln_date_str()}【{self.gz_year()}】{self.sx_year()}年 {self.gz_day()}日 {self.gz_hour()}时"
        ret += "" if (v := self.ln_jie()) == "" else f"{v}节气"
        return ret


if __name__ == "__main__":
    print(Lunar(datetime.datetime.now(pytz.timezone('Asia/Shanghai'))))
    # print(Lunar(datetime.datetime.now()))
    print(Lunar())
