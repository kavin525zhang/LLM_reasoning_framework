...
const parseAggrFunc = (aggrFunc, columns, fieldInfo, replaceMap) => {
    ...
    const result = {
        type: types_1.FilterNodeType.Condition
    };
    if (expr && expr.type === 'aggr_func') {
        console.error('unsupported aggr func!');
    }
    else if (expr && (0, utils_1.checkIsColumnNode)(expr, columns, fieldInfo)) {
        const columnName = (_a = expr.column) !== null && _a !== void 0 ? _a : expr.value;
        result.column = (0, utils_1.replaceString)(columnName, replaceMap);
    }
    result.aggregate = {
        distinct: Boolean(distinct),
        method: (0, utils_1.toFirstUpperCase)(name)
    };
    return result;
};
const parseSQLExpr = (astWhere, columns, fieldInfo, replaceMap, isNot) => {
    ...
    if (type === 'binary_expr') {
        ...
        if (['AND', 'OR'].includes(operator)) {
            result.type = operator === 'AND' ? types_1.FilterNodeType.And : types_1.FilterNodeType.Or;
            result.conditions = [
                parseSQLExpr(left, columns, fieldInfo, replaceMap),
                parseSQLExpr(right, columns, fieldInfo, replaceMap)
            ];
        }
        else if (Object.values(types_1.FilterOperator)
            .map(v => v.toUpperCase())
            .includes(operator)) {
            result.type = types_1.FilterNodeType.Condition;
            const columnNode = [left, right].find(n => (0, utils_1.checkIsColumnNode)(n, columns, fieldInfo));
            if (columnNode) {
                const columnName = (_a = columnNode.column) !== null && _a !== void 0 ? _a : columnNode.value;
                result.column = (0, utils_1.replaceString)(columnName, replaceMap);
            }
            const valueNode = [left, right].find(n => !(0, utils_1.checkIsColumnNode)(n, columns, fieldInfo) && n.type !== 'aggr_func');
            if (valueNode) {
                const valueName = valueNode.value;
                if (!(0, vutils_1.isArray)(valueName)) {
                    result.value = (0, utils_1.replaceString)(valueName, replaceMap);
                }
                else {
                    result.value = valueName.map(v => (0, utils_1.replaceString)(v.value, replaceMap));
                }
            }
...
const where = (query, context) => {
    ...
    return {
        ...query,
        where: whereList.conditions ? whereList : { not: false, type: types_1.FilterNodeType.And, conditions: [whereList] }
    };
};
exports.where = where;
const groupBy = (query, context) => {
    ...
    return {
        ...query,
        groupBy: (groupby !== null && groupby !== void 0 ? groupby : []).map((group) => { var _a; return (0, utils_1.replaceString)((_a = group.column) !== null && _a !== void 0 ? _a : group.value, replaceMap); })
    };
};
exports.groupBy = groupBy;
const select = (query, context) => {
    const { ast, fieldInfo, replaceMap } = context;
    ...
    return {
        ...
        select: {
            columns: (columns !== null && columns !== void 0 ? columns : [])
                ...
                if ((0, utils_1.checkIsColumnNode)(expr, columnAlias, fieldInfo)) {
                    result.column = (0, utils_1.replaceString)((_a = expr.column) !== null && _a !== void 0 ? _a : expr.value, replaceMap);
                }
                ...
                if (as) {
                    result.alias = (0, utils_1.replaceString)(as, replaceMap);
                }
                return result;
            })
                .filter(c => c.column),
...
const having = (query, context) => {
    ...
    return {
        ...query,
        having: havingList.conditions ? havingList : { not: false, type: types_1.FilterNodeType.And, conditions: [havingList] }
    };
};
exports.having = having;
const orderBy = (query, context) => {
    const { ast, fieldInfo, replaceMap } = context;
    const { orderby } = ast;
    if (!orderby) {
        return query;
    ...
    return {
        ...
        orderBy: (orderby !== null && orderby !== void 0 ? orderby : []).map(orderInfo => {
            ...
            if ((0, utils_1.checkIsColumnNode)(expr, query.select.columns, fieldInfo)) {
                const columnName = (_a = expr.column) !== null && _a !== void 0 ? _a : expr.value;
                result.column = (0, utils_1.replaceString)(columnName, replaceMap);
            }
            else {
            ...
            return {
                type: type ? (0, utils_1.toFirstUpperCase)(type) : types_1.OrderType.Asc,
                ...result
            };
        })
    };
