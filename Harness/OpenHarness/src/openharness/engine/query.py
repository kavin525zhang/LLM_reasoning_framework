...
from openharness.log_utils import logger as log
...
async def run_query(
    ...
    while context.max_turns is None or turn_count < context.max_turns:
        ...
        # final_message.tool_uses[0].name="skill"
        # final_message.tool_uses[0].input={'name': 'tavily-search'}

        tool_calls = final_message.tool_uses
        """
        [ToolUseBlock(type='tool_use', 
                      id='chatcmpl-tool-8e2c3ef8119a0b5f', 
                      name='tavily_search', 
                      input={'query': '星环科技2026年一季度营业收入'})]
        """
        log.info("tool_calls11111:{}".format(tool_calls))
        if len(tool_calls) == 1:
...
async def _execute_tool_call(
    ...
    log.info("tool_name:{}, context.tool_registry:{}".format(tool_name, context.tool_registry.list_tools()))
